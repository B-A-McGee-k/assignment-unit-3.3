...
console.log('The item list is now', supplyChanges)
...
for (let item = 0; item<supplyChanges.length; item++) {
  ...
  if (supplyChanges[item] === 0) {
    zeroes.push(supplyChanges[item])
  } else if (supplyChanges[item]>0) {
    positives.push(supplyChanges[item])
  } else {
    negatives.push(supplyChanges[item])
  }
}
...
supplyChanges.sort(compareNumbers);
console.log(supplyChanges);
for (item of supplyChanges) {
  if (supplyChanges[item] === 0) {
    stretchZeroes.push(supplyChanges[item]);
  } else if (supplyChanges[item] > 0) {
    stretchPositives.push(supplyChanges[item]);
  } else if (supplyChanges[item] < 0) {
    stretchNegatives.push(supplyChanges[item]);
  }
}
